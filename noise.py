...
def GN_AR1_recurrent_Laplace(rho=0.9, sigma1=0.5, size=100):
    """
    AR(1) noise with Laplacian innovations.
    """
    noise = N.zeros(size)

    # initial value
    noise[0] = N.random.laplace(0, sigma1)

    # recurrence
    for t in range(1, size):
        e = N.random.laplace(0, sigma1)
        noise[t] = rho * noise[t-1] + e
